...
Mat gausBlur(Mat& img, int radius, float strenght){
...
vector<float> calculateGradients(Mat& img){

    // Get gradient in y direction
    Mat kernelVert = edgeVertical();
    Mat yGradient = convolve(img, kernelVert);

    // Get gradient in x direction
    Mat kernelHort = edgeHorizontal();
    Mat xGradient = convolve(img, kernelHort);

    int rows = xGradient.rows, cols = xGradient.cols;
    float magnitude, angleRad, angleDeg;

    Mat result(rows, cols, CV_8UC3, Scalar(0));

    int remainderX = rows % 8;
    int remainderY = cols % 8;

    int histogram[9] = {0, 20, 40, 60, 80, 100, 120, 140, 160};
    float histogramVote[9] = {0.0};
    vector<float> histograms;

    for(int x = 0; x < rows - remainderX; x += 8){
        for(int y = 0; y < cols - remainderY; y += 8){
            for(int i = 0; i < 8; i++){
                for(int j = 0; j < 8; j++){
                    ...
                    // Calculate the magnitude and the angle of the gradient
                    magnitude = sqrt((xVal * xVal) + (yVal * yVal));
                    angleDeg = 0;

                    if (xVal != 0){
                        angleRad = atan((float)yVal / xVal);
                        angleDeg = round(angleRad * 180.0 / pi);
                    }

                    // Loop through the histogram bins, and calculate the distance from the current angle
                    float fLowest = INFINITY;
                    float sLowest = INFINITY;
                    int fBin = 0;
                    int sBin = 0;

                    for(int i = 0; i < 9; i++){
                        float dist = abs(histogram[i] - angleDeg);

                        if (angleDeg > 160.0){
                            float secondDist = abs(180.0 - angleDeg);
                    ...
                    // Distribute the magnitude based on the distances
                    float distSum = fLowest + sLowest;
                    histogramVote[fBin]+= (1.0 - fLowest / distSum) * magnitude;
                    histogramVote[sBin] += (1.0 - sLowest / distSum) * magnitude;
                }
            }
            for(int i = 0; i < 9; i ++){
                histograms.push_back(histogramVote[i]);
            }
        }
    }
